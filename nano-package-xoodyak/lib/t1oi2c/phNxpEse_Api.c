...
static int phNxpEse_readPacket(void *conn_ctx, void *pDevHandle,
                               uint8_t *pBuffer, int nNbBytesToRead);
...
ESESTATUS phNxpEse_init(void *conn_ctx, phNxpEse_initParams initParams,
  ...
  phNxpEse_Context_t *nxpese_ctxt =
      (conn_ctx == NULL) ? &gnxpese_ctxt : (phNxpEse_Context_t *)conn_ctx;
  bool_t status = FALSE;
 ...
 *
 * Description      This function validate ESE state & C-APDU data before
 *sending it to 7816 protocol
 *
 * param[in]       connection context
 ...
 *
 * Description      This function read out complete data from  SE FIFO read
 *buffer interface (e.g. I2C) using the  driver interface. Just to make sure
 *that if host is unable to read complete data during previous transaction
 *
...
/* 
  ...
  memset(readBuf, 0, sizeof(t10i2c_tempBuf));
  ret = 0;
  if (ret < 0) {
    /* Do nothing as nothing to read*/
    SMLOG_I("Nothing to read from buffer\n");

  } else {
    SMLOG_I("Read buffer cleared\n");
...
static int phNxpEse_readPacket(void *conn_ctx, void *pDevHandle,
  ...
  do {
    ...
    if ((pBuffer[0] == 0x00) &&
      ...
      SMLOG_D("Reading %d for invalid frame and host SE out of sync\n",
              numBytesToRead);
      ...
      SMLOG_D("Reading %d for complete data + 2 byte CRC\n",
    ...
    if (poll_sof_chained_delay == 1) {
      T_SMLOG_D("%s Chained Pkt, delay read %dms", __FUNCTION__,
                ESE_POLL_DELAY_MS * CHAINED_PKT_SCALER);
      sm_sleep(ESE_POLL_DELAY_MS);
    } else {
      T_SMLOG_D("%s Normal Pkt, delay read %dms", __FUNCTION__,
                ESE_POLL_DELAY_MS * NAD_POLLING_SCALER);
      sm_sleep(ESE_POLL_DELAY_MS);
    }
 ...
 *
 ******************************************************************************/
ESESTATUS phNxpEse_WriteFrame(void *conn_ctx, uint32_t data_len,
  ...
  if (nxpese_ctxt->EseLibStatus != ESE_STATUS_CLOSE) {
    dwNoBytesWrRd = phPalEse_i2c_write(
        nxpese_ctxt->pDevHandle, nxpese_ctxt->p_cmd_data, nxpese_ctxt->cmd_len);
    if (-1 == dwNoBytesWrRd) {
 ...
 *
 ******************************************************************************/
void *phNxpEse_memalloc(uint32_t size) { return sm_malloc(size); }
