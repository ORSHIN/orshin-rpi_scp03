 ...
 * [!] NOTE: Slight modifications were carried out to the present file with
...
smStatus_t Se05x_API_SessionOpen(pSe05xSession_t session_ctx) {
  ...
  if (session_ctx->session_resume == 1) {
    ret = smComT1oI2C_Open(session_ctx->conn_context, ESE_MODE_RESUME, 0x00,
                           session_ctx->apdu_buffer, &buff_len);
  } else {
    ret = smComT1oI2C_Open(session_ctx->conn_context, ESE_MODE_NORMAL, 0x00,
                           session_ctx->apdu_buffer, &buff_len);
  }
  ENSURE_OR_GO_CLEANUP(SM_OK == ret);
  ...
  if (!session_ctx->session_resume) {
    ...
    buff_len = sizeof(session_ctx->apdu_buffer);
    ret = smComT1oI2C_TransceiveRaw(session_ctx->conn_context,
                                    session_ctx->apdu_buffer, tx_len,
                                    session_ctx->apdu_buffer, &buff_len);
    if (ret != SM_OK) {
  ...
  if (session_ctx->session_resume) {
    ...
    // ret = Se05x_API_SCP03_CreateSession(session_ctx); // Originally
    // uncommented.
    ret = SM_OK;  // Added by me.
  }
#endif  // #ifdef WITH_PLATFORM_SCP03
...
smStatus_t Se05x_API_WriteECKey(pSe05xSession_t session_ctx,
  ...
  tlvHeader_t hdr = {{kSE05x_CLA, kSE05x_INS_WRITE | ins_type,
                      kSE05x_P1_EC | key_part, kSE05x_P2_DEFAULT}};
  size_t cmdbufLen = 0;
  ...
  tlvRet = TLVSET_Se05xPolicy("policy", &pCmdbuf, &cmdbufLen, kSE05x_TAG_POLICY,
                              policy);
  if (0 != tlvRet) {
    goto cleanup;
  }
  tlvRet = TLVSET_MaxAttemps("maxAttempt", &pCmdbuf, &cmdbufLen,
                             kSE05x_TAG_MAX_ATTEMPTS, maxAttempt);
  if (0 != tlvRet) {
    goto cleanup;
  }
  tlvRet =
      TLVSET_U32("object id", &pCmdbuf, &cmdbufLen, kSE05x_TAG_1, objectID);
  if (0 != tlvRet) {
    goto cleanup;
  }
  tlvRet =
      TLVSET_ECCurve("curveID", &pCmdbuf, &cmdbufLen, kSE05x_TAG_2, curveID);
  if (0 != tlvRet) {
    goto cleanup;
  }
  tlvRet = TLVSET_u8bufOptional("privKey", &pCmdbuf, &cmdbufLen, kSE05x_TAG_3,
                                privKey, privKeyLen);
  if (0 != tlvRet) {
    goto cleanup;
  }
  tlvRet = TLVSET_u8bufOptional("pubKey", &pCmdbuf, &cmdbufLen, kSE05x_TAG_4,
                                pubKey, pubKeyLen);
  if (0 != tlvRet) {
    goto cleanup;
  }
  ...
  retStatus =
      DoAPDUTx(session_ctx, &hdr, session_ctx->apdu_buffer, cmdbufLen, 0);
...
smStatus_t Se05x_API_ReadObject(pSe05xSession_t session_ctx, uint32_t objectID,
  ...
  smStatus_t retStatus = SM_NOT_OK;
  tlvHeader_t hdr = {
      {kSE05x_CLA, kSE05x_INS_READ, kSE05x_P1_DEFAULT, kSE05x_P2_DEFAULT}};
  size_t cmdbufLen = 0;
  uint8_t *pCmdbuf = NULL;
  int tlvRet = 0;
  ...
  tlvRet =
      TLVSET_U16Optional("offset", &pCmdbuf, &cmdbufLen, kSE05x_TAG_2, offset);
  if (0 != tlvRet) {
    goto cleanup;
  }
  tlvRet =
      TLVSET_U16Optional("length", &pCmdbuf, &cmdbufLen, kSE05x_TAG_3, length);
  if (0 != tlvRet) {
    goto cleanup;
  }
  ...
  if (retStatus == SM_OK) {
    retStatus = SM_NOT_OK;
    size_t rspIndex = 0;
    tlvRet = tlvGet_u8buf(pRspbuf, &rspIndex, rspbufLen, kSE05x_TAG_1, data,
                          pdataLen); /*  */
    if (0 != tlvRet) {
      goto cleanup;
    }
...
smStatus_t Se05x_API_GetVersion(pSe05xSession_t session_ctx,
  ...
  smStatus_t retStatus = SM_NOT_OK;
  tlvHeader_t hdr = {
      {kSE05x_CLA, kSE05x_INS_MGMT, kSE05x_P1_DEFAULT, kSE05x_P2_VERSION}};
  int tlvRet = 0;
  uint8_t *pRspbuf = NULL;
  size_t rspbufLen = 0;
  ...
  retStatus = DoAPDUTxRx(session_ctx, &hdr, session_ctx->apdu_buffer, 0,
                         pRspbuf, &rspbufLen, 0);
  if (retStatus == SM_OK) {
    retStatus = SM_NOT_OK;
    size_t rspIndex = 0;
    tlvRet = tlvGet_u8buf(pRspbuf, &rspIndex, rspbufLen, kSE05x_TAG_1,
                          pappletVersion, appletVersionLen);
    if (0 != tlvRet) {
      goto cleanup;
    }
    if ((rspIndex + 2) == rspbufLen) {
...
smStatus_t Se05x_API_ECDSASign(pSe05xSession_t session_ctx, uint32_t objectID,
  ...
  smStatus_t retStatus = SM_NOT_OK;
  tlvHeader_t hdr = {
      {kSE05x_CLA, kSE05x_INS_CRYPTO, kSE05x_P1_SIGNATURE, kSE05x_P2_SIGN}};
  size_t cmdbufLen = 0;
  uint8_t *pCmdbuf = NULL;
  int tlvRet = 0;
  uint8_t *pRspbuf = NULL;
  ...
  tlvRet = TLVSET_ECSignatureAlgo("ecSignAlgo", &pCmdbuf, &cmdbufLen,
                                  kSE05x_TAG_2, ecSignAlgo);
  if (0 != tlvRet) {
    goto cleanup;
  }
  tlvRet = TLVSET_u8bufOptional("inputData", &pCmdbuf, &cmdbufLen, kSE05x_TAG_3,
...
smStatus_t Se05x_API_ECDSAVerify(pSe05xSession_t session_ctx, uint32_t objectID,
  ...
  smStatus_t retStatus = SM_NOT_OK;
  tlvHeader_t hdr = {
      {kSE05x_CLA, kSE05x_INS_CRYPTO, kSE05x_P1_SIGNATURE, kSE05x_P2_VERIFY}};
  size_t cmdbufLen = 0;
  uint8_t *pCmdbuf = NULL;
  int tlvRet = 0;
  uint8_t *pRspbuf = NULL;
  ...
  tlvRet = TLVSET_u8bufOptional("signature", &pCmdbuf, &cmdbufLen, kSE05x_TAG_5,
  ...
  if (retStatus == SM_OK) {
    retStatus = SM_NOT_OK;
    size_t rspIndex = 0;
    tlvRet = tlvGet_Result(pRspbuf, &rspIndex, rspbufLen, kSE05x_TAG_1,
                           presult); /* - */
    if (0 != tlvRet) {
      goto cleanup;
    }
...
smStatus_t Se05x_API_CheckObjectExists(pSe05xSession_t session_ctx,
  ...
  smStatus_t retStatus = SM_NOT_OK;
  tlvHeader_t hdr = {
      {kSE05x_CLA, kSE05x_INS_MGMT, kSE05x_P1_DEFAULT, kSE05x_P2_EXIST}};
  size_t cmdbufLen = 0;
  uint8_t *pCmdbuf = NULL;
  int tlvRet = 0;
  uint8_t *pRspbuf = NULL;
...
smStatus_t Se05x_API_WriteBinary(pSe05xSession_t session_ctx,
  ...
  smStatus_t retStatus = SM_NOT_OK;
  tlvHeader_t hdr = {
      {kSE05x_CLA, kSE05x_INS_WRITE, kSE05x_P1_BINARY, kSE05x_P2_DEFAULT}};
  size_t cmdbufLen = 0;
  uint8_t *pCmdbuf = NULL;
  int tlvRet = 0;
  ...
  tlvRet = TLVSET_u8bufOptional("input data", &pCmdbuf, &cmdbufLen,
                                kSE05x_TAG_4, inputData, inputDataLen);
  if (0 != tlvRet) {
    goto cleanup;
  }
  retStatus =
...
smStatus_t Se05x_API_ECDHGenerateSharedSecret(
  ...
  smStatus_t retStatus = SM_NOT_OK;
  tlvHeader_t hdr = {
      {kSE05x_CLA, kSE05x_INS_CRYPTO, kSE05x_P1_EC, kSE05x_P2_DH}};
  size_t cmdbufLen = 0;
  uint8_t *pCmdbuf = NULL;
  int tlvRet = 0;
  uint8_t *pRspbuf = NULL;
  ...
  tlvRet = TLVSET_u8bufOptional("pubKey", &pCmdbuf, &cmdbufLen, kSE05x_TAG_2,
                                pubKey, pubKeyLen);
  if (0 != tlvRet) {
    goto cleanup;
  }
...
smStatus_t Se05x_API_CipherOneShot(
  ...
  smStatus_t retStatus = SM_NOT_OK;
  tlvHeader_t hdr = {
      {kSE05x_CLA, kSE05x_INS_CRYPTO, kSE05x_P1_CIPHER, operation}};
  size_t cmdbufLen = 0;
  uint8_t *pCmdbuf = NULL;
  int tlvRet = 0;
  uint8_t *pRspbuf = NULL;
  ...
  tlvRet = TLVSET_CipherMode("cipherMode", &pCmdbuf, &cmdbufLen, kSE05x_TAG_2,
                             cipherMode);
  if (0 != tlvRet) {
    goto cleanup;
  }
  ...
  tlvRet =
      TLVSET_u8bufOptional("IV", &pCmdbuf, &cmdbufLen, kSE05x_TAG_4, IV, IVLen);
  if (0 != tlvRet) {
    goto cleanup;
  }
...
smStatus_t Se05x_API_WriteSymmKey(pSe05xSession_t session_ctx,
  ...
  smStatus_t retStatus = SM_NOT_OK;
  tlvHeader_t hdr = {
      {kSE05x_CLA, kSE05x_INS_WRITE | ins_type, type, kSE05x_P2_DEFAULT}};
  size_t cmdbufLen = 0;
  uint8_t *pCmdbuf = NULL;
  int tlvRet = 0;
  ...
  tlvRet = TLVSET_u8bufOptional("key value", &pCmdbuf, &cmdbufLen, kSE05x_TAG_3,
...
smStatus_t Se05x_API_DeleteSecureObject(pSe05xSession_t session_ctx,
                                        uint32_t objectID) {
  smStatus_t retStatus = SM_NOT_OK;
  tlvHeader_t hdr = {{kSE05x_CLA, kSE05x_INS_MGMT, kSE05x_P1_DEFAULT,
                      kSE05x_P2_DELETE_OBJECT}};
  size_t cmdbufLen = 0;
  uint8_t *pCmdbuf = NULL;
  int tlvRet = 0;
