 ...
 * [!] NOTE: Modifications were carried out to the present file with respect to
 * the original one.
 */
...
uint8_t scp03_enc_key[16] = {0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD,
                             0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD, 0x00, 0x01};
uint8_t scp03_mac_key[16] = {0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD, 0xAB, 0xCD,
...
int ex_generate_nist256_key(pSe05xSession_t session_ctx) {
  smStatus_t status;
  uint32_t keyID = TEST_ID_BASE + __LINE__ + cnt;
  ...
  /* Generate key */
  status = Se05x_API_WriteECKey(session_ctx, NULL, 0, keyID, curveID, NULL, 0,
                                NULL, 0, kSE05x_INS_NA, kSE05x_KeyPart_Pair);
  if (status != SM_OK) {
...
int ex_set_get_nist256_key(pSe05xSession_t session_ctx) {
  ...
  status = Se05x_API_WriteECKey(session_ctx, NULL, 0, keyID, curveID,
                                nist256PrivKey, sizeof(nist256PrivKey),
                                nist256PubKey, sizeof(nist256PubKey),
                                kSE05x_INS_NA, kSE05x_KeyPart_Pair);
  if (status != SM_OK) {
    SMLOG_E("Error in Se05x_API_WriteECKey \n");
    goto exit;
  }
  ...
  if ((key_buflen != sizeof(nist256PubKey)) ||
      (memcmp(key_buf, nist256PubKey, key_buflen) != 0)) {
...
int ex_nist256_sign_verify(pSe05xSession_t session_ctx) {
  ...
  status =
      Se05x_API_ECDSASign(session_ctx, keyID, kSE05x_ECSignatureAlgo_SHA_256,
  ...
  status =
      Se05x_API_ReadObject(session_ctx, keyID, 0, 0, pub_key, &pub_key_len);
  if (status != SM_OK) {
    SMLOG_E("Error in Se05x_API_ReadObject \n");
    goto exit;
  }
  ...
  /* Set public key */
  status = Se05x_API_WriteECKey(session_ctx, NULL, 0, pub_keyID, curveID, NULL,
                                0, pub_key, pub_key_len, kSE05x_INS_NA,
  ...
  status = Se05x_API_ECDSAVerify(
      session_ctx, pub_keyID, kSE05x_ECSignatureAlgo_SHA_256, input, input_len,
      signature, signature_len, &sign_result);
  if (status != SM_OK) {
    SMLOG_E("Error in Se05x_API_ECDSASign \n");
    goto exit;
  }
...
int ex_set_certificate(pSe05xSession_t session_ctx) {
  ...
  for (offset = 0; offset < certificate_len; offset = offset + blk_size) {
    status = Se05x_API_WriteBinary(session_ctx, NULL, keyID, offset, file_size,
                                   certificate + offset, blk_size);
    if (status != SM_OK) {
  ...
  status = Se05x_API_ReadObject(session_ctx, keyID, 200 /*offset*/, blk_size,
                                certificate_ret, &certificate_ret_len);
  if (status != SM_OK) {
    SMLOG_E("Error in Se05x_API_ReadObject \n");
    goto exit;
  }

  if ((certificate_ret_len != blk_size) ||
      (memcmp(certificate + 200, certificate_ret, certificate_ret_len) != 0)) {
...
int ex_ecdh(pSe05xSession_t session_ctx) {
  ...
  /* Calulate ECDH key using key pair at location 'keyID' and public key in
   * buffer 'nist256PubKey' */
  status = Se05x_API_ECDHGenerateSharedSecret(session_ctx, keyID, nist256PubKey,
                                              sizeof(nist256PubKey),
                                              sharedSecret, &sharedSecret_len);
  if (status != SM_OK) {
...
int ex_aes(pSe05xSession_t session_ctx, SE05x_CipherMode_t cipherMode) {
  ...
  /* Write symm key */
  status = Se05x_API_WriteSymmKey(session_ctx, NULL, 0, keyID,
                                  SE05x_KeyID_KEK_NONE, key, key_len,
  ...
  /* Encrypt data */
  status = Se05x_API_CipherOneShot(session_ctx, keyID, cipherMode, data,
                                   data_len, NULL, 0, enc, &enc_len,
  ...
  /* Decrypt data */
  status = Se05x_API_CipherOneShot(session_ctx, keyID, cipherMode, enc, enc_len,
                                   NULL, 0, dec, &dec_len,
...
int ex_nist256_sign_policy(pSe05xSession_t session_ctx) {
  ...
  /* Set Read, Write, Generate, Read, Delete policy. No sign / verify policy is
   * set. */
  ...
  /* Generate nist256 key */
  status =
      Se05x_API_WriteECKey(session_ctx, &eccKeyPolicy, 0, keyID, curveID, NULL,
                           0, NULL, 0, kSE05x_INS_NA, kSE05x_KeyPart_Pair);
  if (status != SM_OK) {
    SMLOG_E("Error in Se05x_API_WriteECKey \n");
    goto exit;
  }
  ...
  if (status != SM_ERR_ACCESS_DENIED_BASED_ON_POLICY) {
    SMLOG_E(
        "Se05x_API_ECDSASign should have failed with "
        "'SM_ERR_ACCESS_DENIED_BASED_ON_POLICY' \n");
    goto exit;
  }
...
void ex_set_scp03_keys(pSe05xSession_t session_ctx) {
  ...
  session_ctx->pScp03_dek_key =
      NULL;  // DEK key is required only for key rotation example
